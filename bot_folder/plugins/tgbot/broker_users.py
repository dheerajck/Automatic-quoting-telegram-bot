...
async def add_broker_user_handler(client, message):
    ...
    if user_object:
        ...
        try:
            await BrokerChannel.objects.acreate(group_id=user_object.id, title=name, is_user=True)
...
async def remove_broker_user_handler(client, message):
    ...
    if user_object:
        name = user_object.first_name + (user_object.last_name or "")
        await BrokerChannel.objects.filter(group_id=user_object.id, is_user=True).adelete()
